...
// mutations
const mutations = {
    cleanCurrentSysTasklogByState (state, body) {
        state.current = {};
    },
    querySysTasklogSuccess (state, body) {
    ...
    findSysTasklogByState (state, data) {
    ...
    findBySysTasklogIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveSysTasklogSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateSysTasklogSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysTasklogSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysTasklogSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
