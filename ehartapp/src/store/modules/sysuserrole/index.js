...
// mutations
const mutations = {
    cleanCurrentSysUserRoleByState (state, body) {
        state.current = {};
    },
    querySysUserRoleSuccess (state, body) {
    ...
    findSysUserRoleByState (state, data) {
    ...
    findBySysUserRoleIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveSysUserRoleSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateSysUserRoleSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysUserRoleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysUserRoleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
