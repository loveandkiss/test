...
// mutations
const mutations = {
    cleanCurrentSysParameterByState (state, body) {
        state.current = {};
    },
    querySysParameterSuccess (state, body) {
    ...
    findSysParameterByState (state, data) {
    ...
    findBySysParameterIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveSysParameterSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateSysParameterSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysParameterSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysParameterSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
