...
// mutations
const mutations = {
    cleanCurrentVehicleWeightByState (state, body) {
        state.current = {};
    },
    queryVehicleWeightSuccess (state, body) {
    ...
    findVehicleWeightByState (state, data) {
    ...
    findByVehicleWeightIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveVehicleWeightSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateVehicleWeightSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteVehicleWeightSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteVehicleWeightSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
