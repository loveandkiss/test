...
// mutations
const mutations = {
    cleanCurrentSysTaskByState (state, body) {
        state.current = {};
    },
    querySysTaskSuccess (state, body) {
    ...
    findBySysTaskIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    updateSysTaskSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysTaskSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysTaskSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
