...
// mutations
const mutations = {
    cleanCurrentVehicleDetailByState (state, body) {
        state.current = {};
    },
    queryVehicleDetailSuccess (state, body) {
    ...
    findVehicleDetailByState (state, data) {
    ...
    findByVehicleDetailIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveVehicleDetailSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateVehicleDetailSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteVehicleDetailSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteVehicleDetailSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
