...
// mutations
const mutations = {
    cleanCurrentVehicleByState (state, body) {
        state.current = {};
    },
    queryVehicleSuccess (state, body) {
    ...
    findByVehicleIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    updateVehicleSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteVehicleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteVehicleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
