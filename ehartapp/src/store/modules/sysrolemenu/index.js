...
// mutations
const mutations = {
    cleanCurrentSysRoleMenuByState (state, body) {
        state.current = {};
    },
    querySysRoleMenuSuccess (state, body) {
    ...
    findSysRoleMenuByState (state, data) {
    ...
    findBySysRoleMenuIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    saveSysRoleMenuSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
            state.objs.push(data);
    ...
    updateSysRoleMenuSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysRoleMenuSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysRoleMenuSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
