...
// mutations
const mutations = {
    cleanCurrentSysRoleByState (state, body) {
        state.current = {};
    },
    querySysRoleSuccess (state, body) {
    ...
    findBySysRoleIdSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    updateSysRoleSuccess (state, body) {
        const code = body.code;
        if(code == "200"){
    ...
    deleteSysRoleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            // const data = body.data;
            const data = state.current;
    ...
    batchDeleteSysRoleSuccess(state, body) {
        const code = body.code;
        if(code == "200"){
            const data = body.data;
